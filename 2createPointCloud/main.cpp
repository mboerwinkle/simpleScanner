...
int offset(double vx, double vy, double vz, int x, int y, int z){
	double normalPointVector[3] = {(double)x, (double)y, (double)z};
	norm(normalPointVector);//the vector that points the same direction as the xyz point.
/*	double angle = acos(vx*x+vy*y+vz*z);
	cos(angle)*sqrt(x*x+y*y+z*z);
*/
	return (vx*x+vy*y+vz*z)*sqrt(x*x+y*y+z*z);
}
int findTotalOffset(light* targ, int x, int y, int z){
	int ret = 0;
	for(int cam_idx = 0; cam_idx < CAMERA_COUNT; cam_idx++){
